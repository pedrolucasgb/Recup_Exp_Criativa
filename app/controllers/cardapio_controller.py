...
def fechar_comanda(comanda_id):
    ...
    try:
        ...
        # Validar se a comanda tem itens e valor maior que zero
        if not comanda.itens or len(comanda.itens) == 0:
            flash("Não é possível fechar uma comanda sem itens.", "error")
            return redirect(url_for("auth.dashboard"))
        
        if comanda.valor_total <= 0:
            flash("Não é possível fechar uma comanda com valor R$ 0,00.", "error")
            return redirect(url_for("auth.dashboard"))
...
def processar_pagamento(comanda_id):
    ...
    try:
        ...
        # Validar se a comanda tem valor maior que zero
        if comanda.valor_total <= 0:
            flash("Não é possível faturar uma comanda com valor R$ 0,00.", "error")
            return redirect(url_for("auth.dashboard"))
        
        forma_pagamento = request.form.get("forma_pagamento")
        
        # Validar forma de pagamento
        formas_validas = ['cartao_credito', 'cartao_debito', 'pix']
        if not forma_pagamento or forma_pagamento not in formas_validas:
